...
local tostringx,
  utime,
  exec,
  settings = _G.tostringx, _G.utime, _G.exec, _G.settings
...
BITMAPS.player_blocked_status,
  PALETTES.player_blocked_status = gui.image.load_png_str(BMP_STRINGS.player_blocked_status)
...
base = GLOBAL_SMW_TAS_PARENT_DIR .. 'images/hitbox/'
DBITMAPS.interaction_points[1],
  DBITMAPS.interaction_points_palette = gui.image.load_png('interaction_points_1.png', base)
...
function Options_menu.adjust_lateral_gaps()
  draw.Font = false
  local left_gap,
    right_gap = OPTIONS.left_gap, OPTIONS.right_gap
  local top_gap,
    bottom_gap = OPTIONS.top_gap, OPTIONS.bottom_gap
  ...
  gui.rectangle(
    ...
    1,
    Options_menu.show_menu and COLOUR.warning2 or 0xb0808080
  ) -- unlisted color
  draw.button(
    -draw.Border_left,
    draw.Buffer_middle_y,
    '+',
    function()
...
function Options_menu.display()
  ...
  draw.button(
    ...
    'Lag',
    function()
      Options_menu.current_tab = 'Lag options'
    end,
  ...
  if Options_menu.current_tab == 'Show/hide options' then
    ...
    draw.button(
      ...
      tmp,
      function()
        OPTIONS.display_minor_extended_sprite_info = not OPTIONS.display_minor_extended_sprite_info
      end
    )
    x_pos = x_pos + delta_x + 3
    gui.text(x_pos, y_pos, 'Info')
    ...
    draw.button(
      ...
      tmp,
      function()
        OPTIONS.display_minor_extended_sprite_hitbox = not OPTIONS.display_minor_extended_sprite_hitbox
      end
    )
    x_pos = x_pos + delta_x + 3
    gui.text(x_pos, y_pos, 'Hitbox')
    ...
    draw.button(
      ...
      tmp,
      function()
        OPTIONS.display_debug_minor_extended_sprite = not OPTIONS.display_debug_minor_extended_sprite
      end
    )
    x_pos = x_pos + delta_x + 3
    gui.text(x_pos, y_pos, 'Extra')
  ...
  elseif Options_menu.current_tab == 'Misc options' then
    ...
    gui.text(
      x_pos + 2 * delta_x + 5,
      y_pos,
      fmt('Text opacity: (%.0f%%, %.0f%%)', 100 * draw.Text_max_opacity, 100 * draw.Background_max_opacity)
    )
    y_pos = y_pos + delta_y
    gui.text(
      x_pos,
      y_pos,
      fmt("'%s' and '%s' are hotkeys for this.", OPTIONS.hotkey_decrease_opacity, OPTIONS.hotkey_increase_opacity),
      COLOUR.weak
    )
    y_pos = y_pos + delta_y
    ...
    draw.button(
      x_pos,
      y_pos,
      'Reset Permanent Lateral Paddings',
      function()
  ...
  elseif Options_menu.current_tab == 'Sprite miscellaneous tables' then
    ...
    draw.button(
      ...
      tmp,
      function()
        OPTIONS.display_miscellaneous_sprite_table = not OPTIONS.display_miscellaneous_sprite_table
      end
    )
...
local function sprite_block_interaction_simulator(x_block_left, y_block_bottom)
  ...
  while y_speed < 0 do
    ...
    y,
      y_sub = math.floor(next_total_subpixels / 256), next_total_subpixels % 256
  ...
  for _, positions in ipairs(Layer1_tiles) do
    if
     ...
     then
      ...
      if dup_status then
        local x,
          y = math.floor(positions[1] / 16), math.floor(positions[2] / 16)
        draw.message(fmt('Duplication prediction: %d, %d', x, y), 1000000)

        local xs,
          ys = screen_coordinates(positions[1] + 7, positions[2], Camera_x, Camera_y)
        draw.Font = false
        draw.text(
          draw.AR_x * xs,
          draw.AR_y * ys - 4,
...
special_sprite_property[0x1e] = function(slot) -- Lakitu
  if u8('WRAM', WRAM.sprite_misc_151c + slot) ~= 0 or u8('WRAM', WRAM.sprite_horizontal_direction + slot) ~= 0 then
    ...
    local width,
      height = 0x18 - 1, 0x18 - 1 -- instruction BCS
...
special_sprite_property[0x5f] = function(slot) -- Swinging brown platform (TODO fix it)
  ...
  local x_text,
    y_text,
    height = draw.AR_x * (x_screen + xoff), draw.AR_y * (y_screen + yoff), draw.font_height()
...
special_sprite_property[0x54] = function(slot) -- Revolving door for climbing net
  ...
  if luap.inside_rectangle(Player_x, Player_y, t.x - 8, t.y - 24, t.x + 55, t.y + 55) then
    local extra_x,
      extra_y = screen_coordinates(Player_x, Player_y, Camera_x, Camera_y)
...
special_sprite_property[0x6b] = function(slot) -- Wall springboard (left wall)
  ...
  -- Mario's image
  local xmario,
    ymario = u16('WRAM', 0x7e), u16('WRAM', 0x80)
...
special_sprite_property[0x7b] = function(slot) -- Goal Tape
  ...
  local _,
    y_high = screen_coordinates(0, 0, Camera_x, Camera_y)
  local x_s,
    y_s = screen_coordinates(x_effective, y_low, Camera_x, Camera_y)
  ...
  local x_png,
    y_png = draw.put_on_screen(draw.AR_x * x_s, draw.AR_y * y_s, 18, 6) -- png is 18x6 -- lsnes
...
special_sprite_property[0xa9] = function()
...
special_sprite_property[0x92] = function(slot) -- Splittin' Chuck
  ...
  if u8('WRAM', WRAM.sprite_phase + slot) ~= 5 then
    return
  end
