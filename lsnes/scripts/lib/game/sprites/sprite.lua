...
local function get_swallowed_sprites()
    ...
    for slot = 0, SMW.sprite_max - 1 do
        -- if slot is a Yoshi:
        if u8(WRAM.sprite_number + slot) == 0x35 and u8(WRAM.sprite_status + slot) ~= 0 then
...
local function draw_sprite_hitbox(slot)
    ...
    -- Settings
    local display_hitbox = Display.sprite_hitbox[slot][number].sprite
        and not ABNORMAL_HITBOX_SPRITES[number]
    ...
    if OPTIONS.display_sprite_vs_sprite_hitbox then
        if
            u8(WRAM.sprite_sprite_contact + slot) == 0
            and u8(WRAM.sprite_being_eaten_flag + slot) == 0
            and bit.testn(u8(WRAM.sprite_5_tweaker + slot), 3)
        then
...
local function sprite_info(id, counter, table_position)
    ...
    if OPTIONS.display_sprite_info then
        ...
        draw.text(
            xdraw,
            ydraw,
            fmt('%s#%.2d%s', behind_str, id, contact_str),
            info_color,
            true,
            false,
            0.5,
...
function M.info()
    ...
    if OPTIONS.display_miscellaneous_sprite_table then
        ...
        local x, y =
            draw.AR_x * widget:get_property(tab, 'x'), draw.AR_y * widget:get_property(tab, 'y')
