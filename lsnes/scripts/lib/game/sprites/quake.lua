...
do
    ...
    local function draw_hitbox()
        draw.rectangle(xPos - xCam + hitbox.xoff, yPos - yCam + hitbox.yoff, hitbox.width,
                       hitbox.height, COLOUR.quake_sprite, interact)
    end
    ...
    local function sprite_info(id)
        ...
        draw.text(draw.Buffer_width, draw.Buffer_height + id * font_height,
                  fmt('#%d %d (%d, %d) %d', id, sprite_number, xPos, yPos, quake_timer),
                  COLOUR.quake_sprite, COLOUR.background)
    end
