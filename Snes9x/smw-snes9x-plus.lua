...
local INI_CONFIG_FILENAME = "SMW Utilities by Amaraticando Config.ini"  -- relative to the folder of the script
...
local function create_button(x, y, object, fn, always_on_client, always_on_game, ref_x, ref_y)
    ...
    -- draw the button
	--if Options_menu.current_misc_table then
		--draw_rectangle(x, y, width, height, "#b0b0b0ff", "#00ff0080")  -- unlisted colours
	--else
...
function Options_menu.display()
    ...
    if Options_menu.current_tab == "Show/hide options" then
		...
		tmp = OPTIONS.display_item_box_info and true or " "
        create_button(x_pos, y_pos, tmp, function() OPTIONS.display_item_box_info = not OPTIONS.display_item_box_info end)
        gui.text(x_pos + delta_x + 3, y_pos, "Show Item Box Info?")
        y_pos = y_pos + delta_y
    ...
    elseif Options_menu.current_tab == "Cheats" then
        ...
        if Cheat.allow_cheats then
            ...
            if mouse_onregion(x_pos, y_pos + SNES9X_FONT_HEIGHT - 2, x_pos + 128, y_pos + SNES9X_FONT_HEIGHT + 2) and User_input.leftclick then
                ...
                u8(WRAM.powerup, value)
            end
            draw_rectangle(x_pos + widget_pointer - 1, y_pos + SNES9X_FONT_HEIGHT - 2, 2, 4, "#ff0000a0", COLOUR.warning)  -- unlisted color
            y_pos = y_pos + 2*delta_y
			
			-- Item Box
            value = u8(WRAM.item_box)
            gui.text(x_pos, y_pos, fmt("Item Box:%3d", value))
            create_button(x_pos + 12*SNES9X_FONT_WIDTH + 2, y_pos, "-", function() Cheat.change_address(WRAM.item_box, -1) end)
            create_button(x_pos + 13*SNES9X_FONT_WIDTH + 4, y_pos, "+", function() Cheat.change_address(WRAM.item_box, 1) end)
			local item_box_sprite = value - 141
			if value >= 141 then
				item_box_sprite = value - 141
			else
				item_box_sprite = value + 115
			end
			if item_box_sprite == 0x36 or item_box_sprite == 0xc9 or item_box_sprite == 0xca or item_box_sprite == 0xcb or item_box_sprite == 0xcd
			or item_box_sprite == 0xcd or item_box_sprite == 0xce or item_box_sprite == 0xcf or item_box_sprite == 0xd0 or item_box_sprite == 0xd2
			or item_box_sprite == 0xd3 or item_box_sprite == 0xd5 or item_box_sprite == 0xd7 or item_box_sprite == 0xd9 or item_box_sprite == 0xda
			or item_box_sprite == 0xdd or item_box_sprite == 0xde or item_box_sprite == 0xe0 or item_box_sprite == 0xe1 or item_box_sprite == 0xe2
			or item_box_sprite == 0xe3 or item_box_sprite == 0xe4 or item_box_sprite == 0xe5 or item_box_sprite == 0xe6 or item_box_sprite == 0xe7
			or item_box_sprite == 0xe8 or item_box_sprite == 0xea or item_box_sprite == 0xeb or item_box_sprite == 0xec or item_box_sprite == 0xed
			or item_box_sprite == 0xee or item_box_sprite == 0xef or item_box_sprite == 0xf1 or item_box_sprite == 0xf2 or item_box_sprite == 0xf4
			or item_box_sprite == 0xf5 or item_box_sprite == 0xf6 or item_box_sprite == 0xf7 or item_box_sprite == 0xf8 or item_box_sprite == 0xf9
			or item_box_sprite == 0xfa or item_box_sprite == 0xfb or item_box_sprite == 0xfc or item_box_sprite == 0xfe or item_box_sprite == 0xff then
				draw_text(107, y_pos, fmt("CRASH!"), COLOUR.warning)
			elseif item_box_sprite == 0x7c or item_box_sprite == 0x88 or item_box_sprite == 0xdb or item_box_sprite == 0xe9
			or item_box_sprite == 0xf3 or item_box_sprite == 0xfd then
				draw_text(107, y_pos, fmt("Should not be used!"), COLOUR.warning)
			end
			if value ~= 0 then
				item_box_sprite = string.upper(string.format("%02x", item_box_sprite))
				draw_text(67, y_pos, fmt("Sprite:%s", item_box_sprite))
    ...
    elseif Options_menu.current_tab == "Debug info" then
		...
		tmp = OPTIONS.display_misc_sprite_tables and true or " "
        create_button(x_pos, y_pos, tmp, function() OPTIONS.display_misc_sprite_tables = not OPTIONS.display_misc_sprite_tables
        INI.save_options() end)
        gui.text(x_pos + delta_x + 3, y_pos, "Show miscellaneous sprite tables?")
        y_pos = y_pos + delta_y
        
		if OPTIONS.display_misc_sprite_tables then
			x_pos = 3*x_pos
		
			tmp = OPTIONS.display_misc1 and true or " "
			create_button(x_pos, y_pos, tmp, function() Options_menu.current_misc_table = 1 end)
			gui.text(x_pos + delta_x + 3, y_pos, "Misc 1")
			if Options_menu.current_misc_table == 1 then
				draw_rectangle(x_pos + 1, y_pos + 1, delta_x - 2, delta_y - 6, "#00ff0080", "#00ff00c0")
			end
			y_pos = y_pos + delta_y
			
			tmp = OPTIONS.display_misc2 and true or " "
			create_button(x_pos, y_pos, tmp, function() Options_menu.current_misc_table = 2 end)
			gui.text(x_pos + delta_x + 3, y_pos, "Misc 2")
			if Options_menu.current_misc_table == 2 then
				draw_rectangle(x_pos + 1, y_pos + 1, delta_x - 2, delta_y - 6, "#00ff0080", "#00ff00c0")
			end
			y_pos = y_pos + delta_y
			
			tmp = OPTIONS.display_misc3 and true or " "
			create_button(x_pos, y_pos, tmp, function() Options_menu.current_misc_table = 3 end)
			gui.text(x_pos + delta_x + 3, y_pos, "Misc 3")
			if Options_menu.current_misc_table == 3 then
				draw_rectangle(x_pos + 1, y_pos + 1, delta_x - 2, delta_y - 6, "#00ff0080", "#00ff00c0")
			end
			y_pos = y_pos + delta_y
			
			tmp = OPTIONS.display_misc4 and true or " "
			create_button(x_pos, y_pos, tmp, function() Options_menu.current_misc_table = 4 end)
			gui.text(x_pos + delta_x + 3, y_pos, "Misc 4")
			if Options_menu.current_misc_table == 4 then
				draw_rectangle(x_pos + 1, y_pos + 1, delta_x - 2, delta_y - 6, "#00ff0080", "#00ff00c0")
			end
			y_pos = y_pos + delta_y
			
			tmp = OPTIONS.display_misc5 and true or " "
			create_button(x_pos, y_pos, tmp, function() Options_menu.current_misc_table = 5 end)
			gui.text(x_pos + delta_x + 3, y_pos, "Misc 5")
			if Options_menu.current_misc_table == 5 then
				draw_rectangle(x_pos + 1, y_pos + 1, delta_x - 2, delta_y - 6, "#00ff0080", "#00ff00c0")
			end
			y_pos = y_pos + delta_y
			
			x_pos = x_pos + 40
			y_pos = y_pos - 5*delta_y
			
			tmp = OPTIONS.display_misc6 and true or " "
			create_button(x_pos, y_pos, tmp, function() Options_menu.current_misc_table = 6 end)
			gui.text(x_pos + delta_x + 3, y_pos, "Misc 6")
			if Options_menu.current_misc_table == 6 then
				draw_rectangle(x_pos + 1, y_pos + 1, delta_x - 2, delta_y - 6, "#00ff0080", "#00ff00c0")
			end
			y_pos = y_pos + delta_y
			
			tmp = OPTIONS.display_misc7 and true or " "
			create_button(x_pos, y_pos, tmp, function() Options_menu.current_misc_table = 7 end)
			gui.text(x_pos + delta_x + 3, y_pos, "Misc 7")
			if Options_menu.current_misc_table == 7 then
				draw_rectangle(x_pos + 1, y_pos + 1, delta_x - 2, delta_y - 6, "#00ff0080", "#00ff00c0")
			end
			y_pos = y_pos + delta_y
			
			tmp = OPTIONS.display_misc8 and true or " "
			create_button(x_pos, y_pos, tmp, function() Options_menu.current_misc_table = 8 end)
			gui.text(x_pos + delta_x + 3, y_pos, "Misc 8")
			if Options_menu.current_misc_table == 8 then
				draw_rectangle(x_pos + 1, y_pos + 1, delta_x - 2, delta_y - 6, "#00ff0080", "#00ff00c0")
			end
			y_pos = y_pos + delta_y
			
			tmp = OPTIONS.display_misc9 and true or " "
			create_button(x_pos, y_pos, tmp, function() Options_menu.current_misc_table = 9 end)
			gui.text(x_pos + delta_x + 3, y_pos, "Misc 9")
			if Options_menu.current_misc_table == 9 then
				draw_rectangle(x_pos + 1, y_pos + 1, delta_x - 2, delta_y - 6, "#00ff0080", "#00ff00c0")
			end
			y_pos = y_pos + delta_y
			
			tmp = OPTIONS.display_misc10 and true or " "
			create_button(x_pos, y_pos, tmp, function() Options_menu.current_misc_table = 10 end)
			gui.text(x_pos + delta_x + 3, y_pos, "Misc 10")
			if Options_menu.current_misc_table == 10 then
				draw_rectangle(x_pos + 1, y_pos + 1, delta_x - 2, delta_y - 6, "#00ff0080", "#00ff00c0")
			end
			y_pos = y_pos + delta_y
			...
			tmp = OPTIONS.display_misc11 and true or " "
			create_button(x_pos, y_pos, tmp, function() Options_menu.current_misc_table = 11 end)
			gui.text(x_pos + delta_x + 3, y_pos, "Misc 11")
			if Options_menu.current_misc_table == 11 then
				draw_rectangle(x_pos + 1, y_pos + 1, delta_x - 2, delta_y - 6, "#00ff0080", "#00ff00c0")
			end
			y_pos = y_pos + delta_y
			
			tmp = OPTIONS.display_misc12 and true or " "
			create_button(x_pos, y_pos, tmp, function() Options_menu.current_misc_table = 12 end)
			gui.text(x_pos + delta_x + 3, y_pos, "Misc 12")
			if Options_menu.current_misc_table == 12 then
				draw_rectangle(x_pos + 1, y_pos + 1, delta_x - 2, delta_y - 6, "#00ff0080", "#00ff00c0")
			end
			
			y_pos = y_pos + delta_y
			
			tmp = OPTIONS.display_misc13 and true or " "
			create_button(x_pos, y_pos, tmp, function() Options_menu.current_misc_table = 13 end)
			gui.text(x_pos + delta_x + 3, y_pos, "Misc 13")
			if Options_menu.current_misc_table == 13 then
				draw_rectangle(x_pos + 1, y_pos + 1, delta_x - 2, delta_y - 6, "#00ff0080", "#00ff00c0")
			end
			y_pos = y_pos + delta_y
			
			tmp = OPTIONS.display_misc14 and true or " "
			create_button(x_pos, y_pos, tmp, function() Options_menu.current_misc_table = 14 end)
			gui.text(x_pos + delta_x + 3, y_pos, "Misc 14")
			if Options_menu.current_misc_table == 14 then
				draw_rectangle(x_pos + 1, y_pos + 1, delta_x - 2, delta_y - 6, "#00ff0080", "#00ff00c0")
			end
			y_pos = y_pos + delta_y
			
			tmp = OPTIONS.display_misc15 and true or " "
			create_button(x_pos, y_pos, tmp, function() Options_menu.current_misc_table = 15 end)
			gui.text(x_pos + delta_x + 3, y_pos, "Misc 15")
			if Options_menu.current_misc_table == 15 then
				draw_rectangle(x_pos + 1, y_pos + 1, delta_x - 2, delta_y - 6, "#00ff0080", "#00ff00c0")
			end
			y_pos = y_pos + delta_y
			...
			tmp = OPTIONS.display_misc16 and true or " "
			create_button(x_pos, y_pos, tmp, function() Options_menu.current_misc_table = 16 end)
			gui.text(x_pos + delta_x + 3, y_pos, "Misc 16")
			if Options_menu.current_misc_table == 16 then
				draw_rectangle(x_pos + 1, y_pos + 1, delta_x - 2, delta_y - 6, "#00ff0080", "#00ff00c0")
			end
			y_pos = y_pos + delta_y
			
			tmp = OPTIONS.display_misc17 and true or " "
			create_button(x_pos, y_pos, tmp, function() Options_menu.current_misc_table = 17 end)
			gui.text(x_pos + delta_x + 3, y_pos, "Misc 17")
			if Options_menu.current_misc_table == 17 then
				draw_rectangle(x_pos + 1, y_pos + 1, delta_x - 2, delta_y - 6, "#00ff0080", "#00ff00c0")
			end
			y_pos = y_pos + delta_y
			
			tmp = OPTIONS.display_misc18 and true or " "
			create_button(x_pos, y_pos, tmp, function() Options_menu.current_misc_table = 18 end)
			gui.text(x_pos + delta_x + 3, y_pos, "Misc 18")
			if Options_menu.current_misc_table == 18 then
				draw_rectangle(x_pos + 1, y_pos + 1, delta_x - 2, delta_y - 6, "#00ff0080", "#00ff00c0")
			end
			y_pos = y_pos + delta_y
			
			tmp = OPTIONS.display_misc19 and true or " "
			create_button(x_pos, y_pos, tmp, function() Options_menu.current_misc_table = 19 end)
			gui.text(x_pos + delta_x + 3, y_pos, "Misc 19")
			if Options_menu.current_misc_table == 19 then
				draw_rectangle(x_pos + 1, y_pos + 1, delta_x - 2, delta_y - 6, "#00ff0080", "#00ff00c0")
			end
			y_pos = y_pos + delta_y
			
			OPTIONS.display_current_misc_table = Options_menu.current_misc_table
		end
    end
...
local function player()
	...
	if Cheat.allow_cheats then
		draw_text(table_x, table_y + i*delta_y, fmt("Powerup:%03d", powerup))
	else
		draw_text(table_x, table_y + i*delta_y, fmt("Powerup:%d", powerup))
	end
	i = i + 1
	...
	if OPTIONS.display_item_box_info then
		local item_box_sprite = item_box - 141
		if item_box >= 141 then
			item_box_sprite = item_box - 141
		else
			item_box_sprite = item_box + 115
		end
		draw_text(123, 38, fmt("%03d", item_box))
		if item_box_sprite == 0x36 or item_box_sprite == 0xc9 or item_box_sprite == 0xca or item_box_sprite == 0xcb or item_box_sprite == 0xcd
		or item_box_sprite == 0xcd or item_box_sprite == 0xce or item_box_sprite == 0xcf or item_box_sprite == 0xd0 or item_box_sprite == 0xd2
		or item_box_sprite == 0xd3 or item_box_sprite == 0xd5 or item_box_sprite == 0xd7 or item_box_sprite == 0xd9 or item_box_sprite == 0xda
		or item_box_sprite == 0xdd or item_box_sprite == 0xde or item_box_sprite == 0xe0 or item_box_sprite == 0xe1 or item_box_sprite == 0xe2
		...
		or item_box_sprite == 0xee or item_box_sprite == 0xef or item_box_sprite == 0xf1 or item_box_sprite == 0xf2 or item_box_sprite == 0xf4
		or item_box_sprite == 0xf5 or item_box_sprite == 0xf6 or item_box_sprite == 0xf7 or item_box_sprite == 0xf8 or item_box_sprite == 0xf9
		or item_box_sprite == 0xfa or item_box_sprite == 0xfb or item_box_sprite == 0xfc or item_box_sprite == 0xfe or item_box_sprite == 0xff
		or item_box_sprite == 0x7c or item_box_sprite == 0x88 or item_box_sprite == 0xdb or item_box_sprite == 0xe9 or item_box_sprite == 0xf3
		or item_box_sprite == 0xfd then
			draw_text(148, 20, fmt("!"), COLOUR.warning)
		end
		if item_box ~= 0 then
			item_box_sprite = string.upper(string.format("%02x", item_box_sprite))
			draw_text(140, 20 , item_box_sprite)
		end
    end
...
local function sprite_info(id, counter, table_position)
    ...
    ---**********************************************
    -- The sprite table:	
	local sprite_str = fmt("<%02d> %02x %s%d.%1x(%+.2d) %d.%1x(%+.2d)%s",
                        id, number, special, x, math.floor(x_sub/16), x_speed, y, math.floor(y_sub/16), y_speed, misc_table_space)
	...
	if OPTIONS.display_misc_sprite_tables then
		local misc_1 = u8(WRAM.sprite_miscellaneous1 + id)
		local misc_2 = u8(WRAM.sprite_miscellaneous2 + id)
		local misc_3 = u8(WRAM.sprite_miscellaneous3 + id)
		local misc_4 = u8(WRAM.sprite_miscellaneous4 + id)
		local misc_5 = u8(WRAM.sprite_miscellaneous5 + id)
		local misc_6 = u8(WRAM.sprite_miscellaneous6 + id)
		local misc_7 = u8(WRAM.sprite_miscellaneous7 + id)
		local misc_8 = u8(WRAM.sprite_miscellaneous8 + id)
		local misc_9 = u8(WRAM.sprite_miscellaneous9 + id)
		local misc_10 = u8(WRAM.sprite_miscellaneous10 + id)
		local misc_11 = u8(WRAM.sprite_miscellaneous11 + id)
		local misc_12 = u8(WRAM.sprite_miscellaneous12 + id)
		local misc_13 = u8(WRAM.sprite_miscellaneous13 + id)
		local misc_14 = u8(WRAM.sprite_miscellaneous14 + id)
		local misc_15 = u8(WRAM.sprite_miscellaneous15 + id)
		local misc_16 = u8(WRAM.sprite_miscellaneous16 + id)
		local misc_17 = u8(WRAM.sprite_miscellaneous17 + id)
		local misc_18 = u8(WRAM.sprite_miscellaneous18 + id)
		local misc_19 = u8(WRAM.sprite_miscellaneous19 + id)
		
		if not OPTIONS.display_current_misc_table then
			draw_text(Buffer_width + Border_right, table_position + counter*SNES9X_FONT_HEIGHT, "---", true)
		elseif OPTIONS.display_current_misc_table == 1 then
			draw_text(Buffer_width + Border_right, table_position + counter*SNES9X_FONT_HEIGHT, fmt("%03d", misc_1), true)
		elseif OPTIONS.display_current_misc_table == 2 then
			draw_text(Buffer_width + Border_right, table_position + counter*SNES9X_FONT_HEIGHT, fmt("%03d", misc_2), true)
		elseif OPTIONS.display_current_misc_table == 3 then
			draw_text(Buffer_width + Border_right, table_position + counter*SNES9X_FONT_HEIGHT, fmt("%03d", misc_3), true)
		elseif OPTIONS.display_current_misc_table == 4 then
			draw_text(Buffer_width + Border_right, table_position + counter*SNES9X_FONT_HEIGHT, fmt("%03d", misc_4), true)
		elseif OPTIONS.display_current_misc_table == 5 then
			draw_text(Buffer_width + Border_right, table_position + counter*SNES9X_FONT_HEIGHT, fmt("%03d", misc_5), true)
		elseif OPTIONS.display_current_misc_table == 6 then
			draw_text(Buffer_width + Border_right, table_position + counter*SNES9X_FONT_HEIGHT, fmt("%03d", misc_6), true)
		elseif OPTIONS.display_current_misc_table == 7 then
			draw_text(Buffer_width + Border_right, table_position + counter*SNES9X_FONT_HEIGHT, fmt("%03d", misc_7), true)
		elseif OPTIONS.display_current_misc_table == 8 then
			draw_text(Buffer_width + Border_right, table_position + counter*SNES9X_FONT_HEIGHT, fmt("%03d", misc_8), true)
		elseif OPTIONS.display_current_misc_table == 9 then
			draw_text(Buffer_width + Border_right, table_position + counter*SNES9X_FONT_HEIGHT, fmt("%03d", misc_9), true)
		elseif OPTIONS.display_current_misc_table == 10 then
			draw_text(Buffer_width + Border_right, table_position + counter*SNES9X_FONT_HEIGHT, fmt("%03d", misc_10), true)
		elseif OPTIONS.display_current_misc_table == 11 then
			draw_text(Buffer_width + Border_right, table_position + counter*SNES9X_FONT_HEIGHT, fmt("%03d", misc_11), true)
		elseif OPTIONS.display_current_misc_table == 12 then
			draw_text(Buffer_width + Border_right, table_position + counter*SNES9X_FONT_HEIGHT, fmt("%03d", misc_12), true)
		elseif OPTIONS.display_current_misc_table == 13 then
			draw_text(Buffer_width + Border_right, table_position + counter*SNES9X_FONT_HEIGHT, fmt("%03d", misc_13), true)
		elseif OPTIONS.display_current_misc_table == 14 then
			draw_text(Buffer_width + Border_right, table_position + counter*SNES9X_FONT_HEIGHT, fmt("%03d", misc_14), true)
		elseif OPTIONS.display_current_misc_table == 15 then
			draw_text(Buffer_width + Border_right, table_position + counter*SNES9X_FONT_HEIGHT, fmt("%03d", misc_15), true)
		elseif OPTIONS.display_current_misc_table == 16 then
			draw_text(Buffer_width + Border_right, table_position + counter*SNES9X_FONT_HEIGHT, fmt("%03d", misc_16), true)
		elseif OPTIONS.display_current_misc_table == 17 then
			draw_text(Buffer_width + Border_right, table_position + counter*SNES9X_FONT_HEIGHT, fmt("%03d", misc_17), true)
		elseif OPTIONS.display_current_misc_table == 18 then
			draw_text(Buffer_width + Border_right, table_position + counter*SNES9X_FONT_HEIGHT, fmt("%03d", misc_18), true)
		elseif OPTIONS.display_current_misc_table == 19 then
			draw_text(Buffer_width + Border_right, table_position + counter*SNES9X_FONT_HEIGHT, fmt("%03d", misc_19), true)
		end
		
		if not OPTIONS.display_current_misc_table then
			draw_text(Buffer_width + Border_right, 40, fmt("Misc"), true)
		else
			draw_text(Buffer_width + Border_right, 40, fmt("Misc%d", OPTIONS.display_current_misc_table), true)
		end
	end
...
local function sprites()
    ...
    draw_text(Buffer_width + Border_right, table_position - SNES9X_FONT_HEIGHT, fmt("1st div: %d. Swap: %d%s", -- Snes9x: no extra space at the end
                                                            SPRITE_MEMORY_MAX[smh], swap_slot, misc_table_space), COLOUR.weak, true)
end
...
local function show_counters()
    ...
    local display_counter = function(label, value, default, mult, frame, color)
        ...
        draw_text(0, 105 + (text_counter * height), fmt("%s: %d", label, (value * mult) - frame), color)
    end
...
local function snes9x_buttons()
    ...
    if not Options_menu.show_menu and User_input.mouse_inwindow == 1 then
        ...
        ;
		if Cheat.allow_cheats and Game_mode == SMW.game_mode_level then
			create_button(109, 37, " - ", function() Cheat.change_address(WRAM.item_box, -1) end) -- Item Box cheat
			create_button(135, 37, " + ", function() Cheat.change_address(WRAM.item_box, 1) end)
			if u8(WRAM.powerup) == 2 then
				create_button(44, 72, "-", function() Cheat.change_address(WRAM.powerup, -1) end) -- Powerup cheat
				create_button(49, 72, "+", function() Cheat.change_address(WRAM.powerup, 1) end)
			else
				create_button(44, 64, "-", function() Cheat.change_address(WRAM.powerup, -1) end) -- Powerup cheat
				create_button(49, 64, "+", function() Cheat.change_address(WRAM.powerup, 1) end)
			end
		end
    else
