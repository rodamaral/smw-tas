...
local gui,
  ...
  tostringx,
  bit,
  callback = _G.gui, _G.settings, _G.identify_class, _G.tostringx, _G.bit, _G.callback
...
local function lsnes_screen_info()
  ...
  M.Buffer_width,
    M.Buffer_height = gui.resolution() -- Game area
...
local function draw_text(x, y, text, ...)
  ...
  local x_pos,
    y_pos,
    length = text_position(x, y, text, font_width, font_height, always_on_client, always_on_game, ref_x, ref_y)
...
local function alert_text(x, y, text, text_color, bg_color, always_on_game, ref_x, ref_y)
  ...
  local x_pos,
    y_pos,
    text_length = text_position(x, y, text, font_width, font_height, false, always_on_game, ref_x, ref_y)
...
local function over_text(
  ...
  local x_end,
    y_end,
    length = draw_text(x, y, base, color_base, color_bg, always_on_client, always_on_game, ref_x, ref_y)
...
local function draw_message(message, timeout)
  ...
  Timer.registerfunction(
    timeout,
    function()
      gui.text(0, M.Buffer_height - 2 * LSNES_FONT_HEIGHT, message, COLOUR.text, nil, COLOUR.outline)
    end,
    'draw_message'
  )
end
...
local function pixel(x, y, point, shadow)
  gui.rectangle(M.AR_x * x - 2, M.AR_y * y - 2, M.AR_x * 3, M.AR_y * 3, M.AR_x, shadow or -1, point)
end
...
local function line(x1, y1, x2, y2, scale, ...)
  x1,
    y1,
    x2,
    y2 = scale * x1, scale * y1, scale * x2, scale * y2
  if x1 == x2 then
...
local function rectangle(x, y, w, h, ...)
  x,
    y,
    w,
    h = M.AR_x * x, M.AR_y * y, M.AR_x * w + 2, M.AR_y * h + 2
...
local function button(x, y, object, fn, extra_options)
  ...
  if object_type == 'string' then
    width,
      height = M.font_width(), M.font_height()
    x,
      y,
      width = M.text_position(x, y, object, width, height, always_on_client, always_on_game, ref_x, ref_y)
  elseif object_type == 'userdata' then -- lsnes specific
    width,
      height = object:size()
    x,
      y = M.text_position(x, y, nil, width, height, always_on_client, always_on_game, ref_x, ref_y)
  ...
  table.insert(M.button_list, {x = x, y = y, width = width, height = height, object = object, action = fn})
end
...
M.font_width,
  M.font_height = font_width, font_height
M.copy_bitmap,
  M.copy_dbitmap,
  M.copy_palette = copy_bitmap, copy_dbitmap, copy_palette
M.bitmap_to_dbitmap,
  M.dbitmap_to_bitmap = bitmap_to_dbitmap, dbitmap_to_bitmap
M.palettes_to_adjust,
  M.adjust_palette_transparency = palettes_to_adjust, adjust_palette_transparency
M.increase_opacity,
  M.decrease_opacity = increase_opacity, decrease_opacity
M.put_on_screen,
  M.text_position,
  M.text = put_on_screen, text_position, draw_text
M.alert_text,
  M.over_text,
  M.message = alert_text, over_text, draw_message
M.pixel,
  M.line,
  M.rectangle,
  M.box = pixel, line, rectangle, box
M.button = button
