...
local function drawTile(xpixel, ypixel, xcam, ycam, xplayer, yplayer)
    local bg = (
                xplayer >= xpixel
                and xplayer < xpixel + 0x10
                and yplayer >= ypixel
                and yplayer < ypixel + 0x10
            )
            and 0xa0ffff00
        or -1
...
local function displayPlayer()
    ...
    for i, obj in ipairs {
        ...
        { address = 0x0DC9, word = true, description = 'Y position where Mario should be going to' },
        {
            address = 0x0DCB,
            word = true,
            description = 'X position where Luigi should be going to',
            hide = true,
        },
        {
            address = 0x0DCD,
            word = true,
            description = 'Y position where Luigi should be going to',
            hide = true,
        },
        {
            address = 0x0DCF,
            word = true,
            description = 'Player X speed on the overworld. Added with $7E:13D5',
        },
        {
            address = 0x0DD1,
            word = true,
            description = 'Player Y speed on the overworld. Added with $7E:13D7',
        },
        {
            address = 0x0DD3,
            description = 'Player direction. #$00 = up; #$02 = down; #$04 = left; #$06 = right',
        },
        -- { address = 0x0DD4, description = '' },
        {
            address = 0x0DD5,
            description = 'Used to indicate how a level has been exited',
            important = true,
        },
        ...
        {
            address = 0x13C1,
            description = 'Current Layer 1 overworld tile the player is standing on.',
            important = true,
        },
        {
            address = 0x13C3,
            description = 'Current player submap. #$00 = Main map; #$01 = YI; etc',
            important = true,
        },
        {
            address = 0x13D9,
            description = 'A pointer to various processes running on the overworld',
            important = true,
        },
        { address = 0x1444, description = 'Player is on a level tile', important = true },
        {
            address = 0x1B78,
            description = 'hard coded path should be processed',
            word = true,
            important = true,
        },
        ...
        {
            address = 0x1DED,
            description = 'Last event tile to load to the overworld during a given event',
            word = true,
        },
        ...
        {
            address = 0x1F1F,
            description = "Pointer to Mario's overworld X position. Value is Mario's regular overworld X position divided by #$10 (#16).",
        },
        {
            address = 0x1F21,
            description = "Pointer to Mario's overworld Y position. Value is Mario's regular overworld Y position divided by #$10 (#16).",
        },
        {
            address = 0x1F23,
            description = "Pointer to Luigi's overworld X position. Value is Luigi's regular overworld X position divided by #$10 (#16).",
            hide = true,
        },
        {
            address = 0x1F25,
            description = "Pointer to Luigi's overworld Y position. Value is Luigi's regular overworld Y position divided by #$10 (#16).",
            hide = true,
        },
    } do
