...
function M.draw_layer1(camera_x, camera_y)
    ...
    for number, positions in ipairs(M.layer1) do
        ...
        local right = left + 15
        local bottom = top + 15
        local x_game, y_game = game_coordinates(
            draw.AR_x * left,
            draw.AR_y * top,
            camera_x,
            camera_y
        ...
        if
            draw.AR_x * left > -draw.Border_left - 32
            and draw.AR_y * top > -draw.Border_top - 32
            and draw.AR_x * right < draw.Screen_width + draw.Border_right + 32
            and draw.AR_y * bottom < draw.Screen_height + draw.Border_bottom + 32
        then
            -- Drawings
...
function M.select_tile(x, y, layer_table)
    ...
    if u8(WRAM.game_mode) ~= SMW.game_mode_level then
        return
    end
