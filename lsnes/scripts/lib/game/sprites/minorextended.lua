...
do
    ...
    local function sprite_info(id)
        ...
        local x = luap.signed16(256 * u8('WRAM', WRAM.minorspr_x_high + id) +
                                u8('WRAM', WRAM.minorspr_x_low + id))
        local y = luap.signed16(256 * u8('WRAM', WRAM.minorspr_y_high + id) +
                                u8('WRAM', WRAM.minorspr_y_low + id))
        local xspeed, yspeed = s8('WRAM', WRAM.minorspr_xspeed + id),
                               s8('WRAM', WRAM.minorspr_yspeed + id)
        local x_sub, y_sub = u8('WRAM', WRAM.minorspr_x_sub + id),
                             u8('WRAM', WRAM.minorspr_y_sub + id)
    ...
    function M.sprite_table()
        ...
        for id = 0, SMW.minor_extended_sprite_max - 1 do
            minorspr_number = u8('WRAM', WRAM.minorspr_number + id)
