...
function M.beat_level(is_paused, level_index, level_flag)
  if is_paused and joypad.keys['select'] and (joypad.keys['X'] or joypad.keys['A'] or joypad.keys['B']) then
...
function M.free_movement.apply(previous)
  ...
  if M.free_movement.manipulate_speed then
    ...
    local x_pos,
      y_pos = u16('WRAM', WRAM.x), u16('WRAM', WRAM.y)
...
function M.drag_sprite(id, Game_mode, Sprites_info, Camera_x, Camera_y)
  ...
  local xgame,
    ygame = game_coordinates(User_input.mouse_x - xoff, User_input.mouse_y - yoff, Camera_x, Camera_y)
