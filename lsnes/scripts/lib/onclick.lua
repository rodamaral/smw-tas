...
function M.right()
    ...
    if
        OPTIONS.display_controller_input
        and luap.inside_rectangle(
            User_input.mouse_x,
            User_input.mouse_y,
    ...
    local x_mouse, y_mouse =
        floor(User_input.mouse_x / draw.AR_x) + layer2x,
...
function M.left()
    ...
    for _, field in ipairs(draw.button_list) do
        ...
        if
            keyinput:mouse_onregion(field.x, field.y, field.x + field.width, field.y + field.height)
        then
            field.action()
            config.save_options()
            return
    ...
    if cheat.allow_cheats and cheat.sprite_tweaker_selected_id then
        ...
        print(
            fmt(
                'Edited bit %d of sprite (#%d) tweaker %d (address WRAM+%x).',
                tweaker_bit,
                id,
                tweaker_num,
                address
