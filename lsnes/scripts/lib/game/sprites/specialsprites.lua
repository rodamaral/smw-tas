...
M[0x35] = function(slot, Display) -- Yoshi
  ...
  if not Yoshi_riding_flag and OPTIONS.display_sprite_hitbox and Display.sprite_hitbox[slot][t.number].sprite then
...
M[0x54] = function(slot) -- Revolving door for climbing net
  ...
  if luap.inside_rectangle(Player_x, Player_y, t.x - 8, t.y - 24, t.x + 55, t.y + 55) then
    local extra_x,
      extra_y = screen_coordinates(Player_x, Player_y, xCam, yCam)
...
M[0x6b] = function(slot) -- Wall springboard (left wall)
  ...
  local color = t.info_color
  local Yoshi_riding_flag = u8('WRAM', WRAM.yoshi_riding_flag) ~= 0
  local Player_powerup = u8('WRAM', WRAM.powerup)
...
M[0x7b] = function(slot, Display) -- Goal Tape
  ...
  local _,
    y_high = screen_coordinates(0, 0, xCam, yCam)
  local x_s,
    y_s = screen_coordinates(x_effective, y_low, xCam, yCam)
