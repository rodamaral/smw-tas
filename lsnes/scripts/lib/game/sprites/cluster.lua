...
do
  ...
  local function sprite_info(id)
    ...
    xPos = luap.signed16(256 * u8('WRAM', WRAM.cluspr_x_high + id) + u8('WRAM', WRAM.cluspr_x_low + id))
    yPos = luap.signed16(256 * u8('WRAM', WRAM.cluspr_y_high + id) + u8('WRAM', WRAM.cluspr_y_low + id))
    local clusterspr_timer,
      table_1,
      table_2,
      table_3
    ...
    xScreen,
      yScreen = screen_coordinates(xPos, yPos, xCam, yCam)
    ...
    if OPTIONS.display_debug_cluster_sprite then
      ...
      draw.text(
        ...
        fmt('#%d(%d): (%d, %d) %d, %d, %d', id, clusterspr_number, xPos, yPos, table_1, table_2, table_3),
        color
      )
      counter = counter + 1
    end
  ...
  function M.sprite_table()
    if u8('WRAM', WRAM.cluspr_flag) == 0 then
      return
    end
    ...
    for id = 0, SMW.cluster_sprite_max - 1 do
      clusterspr_number = u8('WRAM', WRAM.cluspr_number + id)
