...
function get_map16_coordinates(index)
  if index < 0 or index > 0x37ff then
    return
  end
...
function M.display_known_tiles()
  ...
  local x_origin,
    y_origin = math.floor(camera_x / 16), math.floor(camera_y / 16)
  for x = x_origin, x_origin + 16 do
    for y = y_origin, y_origin + 14 do
      local num_x,
        num_y,
        kind = get_map16_value(x, y)
      ...
      if kind and M[kind] then
        ...
        if false and 0x1de <= kind and kind <= 0x1fa then
          local _,
            _,
            previous = get_map16_value(x, y - 1)
