...
function OBJDEF.load_decoded_data(filename)
    ...
    handle:close()
    return (text == "") and {} or OBJDEF:decode(text)
end

function OBJDEF.retrieve(filename, previous_data)
...
function OBJDEF.save_options()
    local file, data = OBJDEF.filename, OBJDEF.raw_data
    if not file or not data then print"save_options: <file> and <data> required!"; return end
    
    OBJDEF.save(file, data)
end
