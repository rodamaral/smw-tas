...
local function draw_interaction_points(x_screen, y_screen, left_side, right_side, head, foot, hitbox_type, palette)
...
local function display_position_decimal(props, x_sub_simple, y_sub_simple)
  ...
  draw.text(table_x, table_y + i * delta_y, fmt('Pos (%+d.%s, %+d.%s)',
    store.Player_x,
    x_sub_simple,
    store.Player_y,
    y_sub_simple
  ...
  draw.text(table_x, table_y + i * delta_y, fmt('Speed (%+d(%d.%02.0f), %+d)',
    store.x_speed,
    x_speed_int,
    x_speed_frac,
    store.y_speed
...
local function display_position_hexadecimal(props, x_sub_simple, y_sub_simple)
  ...
  draw.text(table_x, table_y + i * delta_y, fmt('Pos (%+x.%s, %+x.%s)',
  ...
  draw.text(table_x, table_y + i * delta_y, fmt('Speed (%+x.%.2x, %+x)',
    store.x_speed % 0x100,
    store.x_subspeed,
    store.y_speed % 0x100
  ))
...
local function display_player_values(direction, spin_direction, is_caped)
  if OPTIONS.display_player_info then
    ...
    M.draw_blocked_status(props.table_x, props.table_y + props.i * props.delta_y, store.player_blocked_status, store.x_speed, store.y_speed)
    props.i = props.i + 1

    -- Wings timers is the same as the cape
    if (not is_caped and store.cape_fall ~= 0) then
      draw.text(props.table_x,
...
function M.player_hitbox(x, y, is_ducking, powerup, transparency_level, original_palette)
  ...
  draw_hitbox_background(x_screen + left_side, x_screen + right_side, y_screen + head, y_screen + foot, interaction_bg)
  draw_hitbox_sprites(x_screen, y_screen, hitbox_type, mario_line)
  draw_interaction_points(x_screen, y_screen, left_side, right_side, head, foot, hitbox_type, palette)
...
function M.cape_hitbox(spin_direction)
  ...
  local cape_x_screen, cape_y_screen = screen_coordinates(cape_x, cape_y, store.Camera_x, store.Camera_y)
