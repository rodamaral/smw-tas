...
do
  ...
  local function sprite_info(id)
    xPos = luap.signed16(256 * u8('WRAM', WRAM.bouncespr_x_high + id) + u8('WRAM', WRAM.bouncespr_x_low + id))
    yPos = luap.signed16(256 * u8('WRAM', WRAM.bouncespr_y_high + id) + u8('WRAM', WRAM.bouncespr_y_low + id))
    local bounce_timer = u8('WRAM', WRAM.bouncespr_timer + id)

    if OPTIONS.display_debug_bounce_sprite then
      draw.text(xText, yText + height * (id + 1), fmt('#%d:%d (%d, %d)', id, bounce_sprite_number, xPos, yPos))
    end

    if OPTIONS.display_bounce_sprite_info then
      local x_screen,
        y_screen = screen_coordinates(xPos, yPos, xCam, yCam)
      x_screen,
  ...
  function M.sprite_table()
    ...
    -- Debug info
    xText,
      yText = draw.AR_x * 90, draw.AR_y * 37
    if OPTIONS.display_debug_bounce_sprite then
